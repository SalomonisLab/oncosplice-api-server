	...
	if(metadata != undefined)
	{
		...
		metadata.forEach(filter => {
			if(filter.value.indexOf("-") != -1)
			{
				...
				if(heatmapQueries.metadata != undefined){
					heatmapQueries.metadata = " AND ".concat(filter.key).concat(" >= '").concat(numbersToSearch[0]).concat("'");
				}
				else{
					heatmapQueries.metadata = "SELECT * FROM ".concat(cancerTableName).concat("_META WHERE ").concat(filter.key).concat(" >= '").concat(numbersToSearch[0]).concat("'");
				}
				heatmapQueries.metadata = " AND ".concat(filter.key).concat(" <= '").concat(numbersToSearch[1]).concat("'");				
			}
			else
			{
				if(heatmapQueries.metadata != undefined){
                	heatmapQueries.metadata = " AND ".concat(filter.key).concat(" = '").concat(filter.value).concat("'");
                }
                else{
                    heatmapQueries.metadata = "SELECT * FROM ".concat(cancerTableName).concat("_META WHERE ").concat(filter.key).concat(" = '").concat(filter.value).concat("'");
                }
			}
		})
	}
